...
export default function PolshiHub({ poly, kalshi }) {
  ...
  const getMarketData = (platformData, coin, time) => {
    ...
    if (!match) return { price: "N/A", vol: "0" };

    // DRILL: Check multiple platform price fields
    const pRaw = match.yes_bid || 
                 match.price || 
                 (match.markets?.[0]?.outcomePrices ? parseFloat(match.markets[0].outcomePrices[0]) * 100 : null) ||
                 (match.last_price ? match.last_price : null);

    const price = pRaw ? `${parseFloat(pRaw).toFixed(1)}¢` : "N/A";
  ...
  return (
    <div style={{display:'grid', gridTemplateColumns:'1fr', gap:20}}>
      <h2 style={{color:'#3b82f6', borderBottom:'1px solid #1e293b', paddingBottom:10, letterSpacing:'1px'}}>BINARY PRICE TARGETS (YES/NO)</h2>
        ...
        {coins.map(coin => (
          <div key={coin} className="card" style={{borderTop: '3px solid #3b82f6'}}>
            ...
            <table style={{width:'100%', fontSize:'0.8rem', borderCollapse:'collapse'}}>
              ...
              <tbody>
                {intervals.map(time => {
                  const pData = getMarketData(poly, coin, time);
                  const kData = getMarketData(kalshi, coin, time);
                  const pNum = parseFloat(pData.price);
                  const kNum = parseFloat(kData.price);
                  const spread = (!isNaN(pNum) && !isNaN(kNum)) ? Math.abs(pNum - kNum) : 0;
                  
                  return (
                    <tr key={time} style={{height:60, borderTop:'1px solid #1e293b'}}>
                      <td style={{fontWeight:'bold', color:'#94a3b8'}}>{time}</td>
                      <td>
                        <div style={{color:'#4ade80', fontWeight:'bold', fontSize:'1.1rem'}}>{pData.price}</div>
                        <div style={{fontSize:'0.6rem', color:'#64748b', marginTop:2}}>{pData.vol}</div>
                      </td>
                      <td className={spread > 3 ? 'arb-gold' : ''}>
                        <div style={{color:'#ef4444', fontWeight:'bold', fontSize:'1.1rem'}}>{kData.price}</div>
